...
void LinkedBinaryTree::deleteSubtreeMutator(mt19937& rng) {
  PositionList pl = positions();
  vector<Position*> subtrees;

  for(auto& p:pl) {                         // remove root and external
  ...
  int RANDOM_LEAF_INT = randInt(rng, 0, subtrees.size()-1);
  Position delete_leaf = *(subtrees[RANDOM_LEAF_INT]);
  PositionList children;
  preorder(delete_leaf.v, children);

  *delete_leaf = randChoice(rng) ? "a" : "b";
  delete_leaf.setLeft(NULL);
  delete_leaf.setRight(NULL);
...
void LinkedBinaryTree::addSubtreeMutator(mt19937& rng, const int maxDepth) {
  // Find exernal positions to add to
  PositionList pl = positions();
  vector<Position*> external_positions;

  // check if positions are external
  for (auto& p : pl) {
    if (p.isExternal()) {
        external_positions.push_back(&p);
    }
  }

  int RAND_EXTERNAL_LEAF_INT = randInt(rng, 0, external_positions.size()-1);

  // Random external position
  Position add_position = *(external_positions[RAND_EXTERNAL_LEAF_INT]);

  // add branch to add_position
  randomExpressionTree(add_position.v, maxDepth, rng);
}
...
bool LexLessThan(const LinkedBinaryTree& a, const LinkedBinaryTree& b) {
  if (abs(a.getScore() - b.getScore()) < 0.01) {
    if (a.size() > b.size())
      return true;
    else
      return false;
